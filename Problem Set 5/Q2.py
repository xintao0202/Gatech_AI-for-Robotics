...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while change>=tolerance:
        change=0.0
        for i in range(0,len(path)): # not fix end points
            for j in range(len(path[0])):

                if i==0:
                    aux=newpath[i][j]
                    newpath[i][j] += weight_data * (path[i][j] - newpath[i][j]) + \
                     weight_smooth * (newpath[len(path)-1][j] + newpath[i+1][j] - 2.0 * newpath[i][j])
                    change+=abs(aux-newpath[i][j])
                if i==len(path)-1:
                    aux=newpath[i][j]
                    newpath[i][j] += weight_data * (path[i][j] - newpath[i][j]) + \
                     weight_smooth * (newpath[i-1][j] + newpath[0][j] - 2.0 * newpath[i][j])
                    change+=abs(aux-newpath[i][j])
                else:
                    aux=newpath[i][j]
                    newpath[i][j] += weight_data * (path[i][j] - newpath[i][j]) + \
