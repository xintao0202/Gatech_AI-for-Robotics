...
def draw_boxes(background_img, boxes):
    ...
    for key, value in boxes.iteritems():
        ...
        cv2.rectangle(img, (box_tlx, box_tly),
                      (box_brx, box_bry), (0, 0, 255), -1)
        cv2.rectangle(img, (box_tlx, box_tly),
                      (box_brx, box_bry), (0, 0, 0), 1)
        cv2.putText(img, key, (box_blx, box_bly), font,
                    fontScale, fontColor, lineType)
    return img


def execute_student_plan(warehouse, todo, max_distance, max_steering, test_case):
    ...
    background_img = np.ones((height*box_size, width*box_size, 3), np.uint8)
    print state.boxes['0']
    ...
    for y in range(height):
        for x in range(width):
            ...
            cv2.rectangle(background_img, (x*box_size, y*box_size),
                          (x*box_size+box_size, y*box_size+box_size), color, -1)
            cv2.rectangle(background_img, (x*box_size, y*box_size),
                          (x*box_size+box_size, y*box_size+box_size), (0, 0, 0), 1)

    base_image = draw_boxes(background_img, state.boxes)
    last_pos = (np.int(state.robot.x*box_size),
                np.int(-state.robot.y*box_size))
    cv2.circle(base_image, last_pos, np.int(.25*box_size), (255, 0, 0), -1)
    heading_pos = state.robot.find_next_point(0.0, .25)
    heading_pos = (np.int(heading_pos[0] * box_size),
                   np.int(-heading_pos[1] * box_size))
    cv2.line(base_image, last_pos, heading_pos, (0, 0, 255), 1)
    # cv2.imshow('image',base_image)
    # cv2.waitKey(10000)
    if not os.path.exists('cv2_output'):
        os.makedirs('cv2_output')
    if not os.path.exists('cv2_output/tc'+str(test_case)):
        os.makedirs('cv2_output/tc'+str(test_case))
    cv2_output_dir = 'cv2_output/tc'+str(test_case)+'/'
    cv2.imwrite(cv2_output_dir+'image000.png', base_image)
    cv2c = 1
    for action in action_list:
        state.print_to_console()
        state.update_according_to(action)
        new_pos = (np.int(state.robot.x*box_size),
                   np.int(-state.robot.y*box_size))
        cv2.line(background_img, last_pos, new_pos,
                 (128, 128, 128), int(.5*box_size))
        new_state_image = draw_boxes(background_img, state.boxes)

        cv2.circle(new_state_image, new_pos,
                   np.int(.25*box_size), (255, 0, 0), -1)
        heading_pos = state.robot.find_next_point(0.0, .25)
        heading_pos = (
            np.int(heading_pos[0] * box_size), np.int(-heading_pos[1] * box_size))
        cv2.line(new_state_image, new_pos, heading_pos, (0, 0, 255), 1)
        ...
        # cv2.waitKey(1000)
        cv2.imwrite(cv2_output_dir+'image' +
                    str('%03d' % cv2c) + '.png', new_state_image)
        cv2c += 1
    ...
    if num_delivered != len(todo):
        raise Exception(
            "Did not deliver all boxes. Only delivered: {}".format(num_delivered))
...
class State:
    ...
    def _set_initial_state_from(self, warehouse, todo):
        ...
        self.warehouse_limits = {'max_x': float(
            cols), 'min_y': float(-rows), 'segments': []}
    ...
    def _attempt_move(self, steering, distance):
        ...
        try:
            ...
            raise Exception(
                'improperly formatted move destination: {} {}'.format(steering, distance))
    ...
    def _attempt_down(self, x, y):
        ...
        try:
            ...
            if is_legal_down:
                ...
                print "*** Could not down box: destination_is_adjacent = {}, destination_is_open = {}, " \
                      "destination_is_within_warehouse = {}, robot_has_box = {} "\
                    .format(destination_is_adjacent, destination_is_open,
                            destination_is_within_warehouse, robot_has_box)

        except ValueError:
            raise Exception(
                'improperly formatted down destination: {} {}'.format(x, y))
    ...
    def _is_open(self, coordinates, size):
        ...
        for o in self.obstacles:
            if not self._is_outside_obstacle(coordinates, o, size):
                print '*** Space not open - obstacle {} in the way ***'.format(
                    o)
                return False
    ...
    def print_to_console(self):
        ...
        print 'Box state:'
        for box_id, box in self.boxes.iteritems():
            print '\tbox id {}, x = {:6.2f}, y = {:6.2f}'.format(
                box_id, box['ctr_x'], box['ctr_y'])
...
class PartBTestCase(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        with open('results_partB.txt', 'w') as f:
            ...
            f.write("\n-----------")
            f.write(
                '\nTotal Credit: {:.2f} / {}'.format(sum(cls.credit), len(cls.credit)))

    def run_case_with_params(self, params):
        ...
        try:
            ...
            errorMessage = repr(e)
            self.results.append(self.FAIL_TEMPLATE.format(
                message=errorMessage, **params))

        self.assertTrue(accomplished, errorMessage)

        score = params['min_cost']/cost * \
            float(len(delivered))/float(len(params['todo']))
        self.results.append(self.SCORE_TEMPLATE.format(
            cost=cost, score=score, **params))
    ...
    #         'min_cost': 38.
    #     }
    #     score = self.run_case_with_params(params)
...
# # #
    ...
    #     print 'credit: {}'.format(score)
    #
    # # TAs posted on piazza... and I don't like this one
    # @timeout(TIME_LIMIT)
    ...
    #                       '..........................................................................................................................',
    #                       '..........................................................................................................................'],
    #         'todo': [(119, -13.0), (46.0, -9.5), (65.5, -8.5)],
    #         'max_distance': 20.0,
    #         'max_steering': PI/2.+0.01,
    #         'min_cost': 572
    #     }
...
if __name__ == "__main__":
    all_suites = map(lambda x: unittest.TestLoader(
    ).loadTestsFromTestCase(x), [PartBTestCase])
