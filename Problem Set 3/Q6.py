...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if abs(turn)>max_steering_angle:
            raise ValueError, 'larger than max steer angle'
        if forward < 0:
        ...
        #apply noise
        steering_with_noise=random.gauss(turn,self.steering_noise)
        distance_with_noise=random.gauss(forward,self.distance_noise)

        
        beta=distance_with_noise/result.length*(tan(float(steering_with_noise)))
        if abs(beta)<0.001:
            result.x=self.x+(distance_with_noise*cos(self.orientation))
            result.y=self.y+(distance_with_noise*sin(self.orientation))
            result.orientation = (self.orientation+beta)%(2 * pi)
        else:
            R=distance_with_noise/beta
    ...
    # according to
    #           self.bearing_noise
    def sense(self,enable_noise=1): #do not change the name of this function
        Z = []

        # ENTER CODE HERE
        for i in range (len(landmarks)):
            deltaY=landmarks[i][0]-self.y
            detalX=landmarks[i][1]-self.x
            angle=atan2(deltaY,detalX)-self.orientation
            
            if enable_noise:
                angle+=random.gauss(0.0,self.bearing_noise)
            # make sure to normalize the angle
            angle%=2*pi
            Z.append(angle)
