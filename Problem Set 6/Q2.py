...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    Omega=matrix()
    size=(num_landmarks+1)*2 #include initial location
    
    # initialize omega to zero and set initial constraints
    Omega.zero(size,size)
    Omega.value[0][0]=1
    Omega.value[1][1]=1
    
    # initialize Xi to zero and set initial constraints
    Xi=matrix()
    Xi.zero(size,1)
    Xi.value[0][0] = world_size / 2.0
    Xi.value[1][0] = world_size / 2.0   
    
    #use similar code in lesson 19 SLAM Quiz 23, but just 1 dimension
    for k in range(len(data)):
        ...
        #extend Omega and Xi
        # remove element from list, referring https://stackoverflow.com/questions/2793324/is-there-a-simple-way-to-delete-a-list-element-by-value
        new_size=size+2
        ex_indexes = range(0,new_size)
        ex_indexes.remove(2)
        ex_indexes.remove(3)
        
        ex_omega=Omega.expand(new_size,new_size, ex_indexes, ex_indexes)
        ex_xi=Xi.expand(new_size,1, ex_indexes, [0])
       
        # matrix update
        for b in range(4):
            ex_omega.value[b][b]+=1.0/motion_noise
        for b in range(2):
            ex_omega.value[b][b+2]=-1.0/motion_noise
            ex_omega.value[b+2][b]=-1.0/motion_noise
            ex_xi.value[b][0]+=-motion[b]/motion_noise
            ex_xi.value[b+2][0]+=motion[b]/motion_noise
    
        
        
        
        # final fomula using take
        range_take=range(2,new_size)
        matrix_A=ex_omega.take([0,1],range_take)
        matrix_B=ex_omega.take([0,1],[0,1])
        matrix_C=ex_xi.take([0,1],[0])
        Omega_p=ex_omega.take(range_take,range_take)
        Xi_p=ex_xi.take(range_take,[0])
