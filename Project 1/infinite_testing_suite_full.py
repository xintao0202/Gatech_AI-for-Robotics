...
def generate_aberrant_parameters():
 params = [None]
 PI = math.pi
 for i in range(1,10+1):
  d = dict()
  d['test_case'] = i
  d['target_x'] = random.uniform(-20,20)
  d['target_y'] = random.uniform(-20,20)
  d['target_heading'] = random.uniform(0,2*PI) - PI #Range of -pi,pi 
  d['target_period'] = [3, 12][random.randint(0,1)]
  d['target_period'] *= 1 - 2*random.randint(0,1)
  d['target_speed'] = [1, 5][random.randint(0,1)]
  d['target_line_length'] = [1, 16][random.randint(0,1)]
  d['hunter_x'] = random.uniform(-20,20)
  d['hunter_y'] = random.uniform(-20,20)
  d['hunter_heading'] = random.uniform(0,2*PI) - PI # Range of -pi,pi
  params.append(d)
 print 'ABERRANT PARAMETERS'
 return params
...
def generate_parameters():
 ...
 for i in range(1,10+1):
  ...
  d['target_y'] = random.uniform(-20,20)
  d['target_heading'] = random.uniform(0,2*PI) - PI #Range of -pi,pi 
  d['target_period'] = random.randint(3,12)
  d['target_period'] *= 1 - 2*random.randint(0,1)
  d['target_speed'] = random.uniform(1,5)
  d['target_line_length'] = random.randint(1, 16)
  d['hunter_x'] = random.uniform(-20,20)
  d['hunter_y'] = random.uniform(-20,20)
  d['hunter_heading'] = random.uniform(0,2*PI) - PI # Range of -pi,pi
  params.append(d)
...
def generate_global_parameters():
 return generate_aberrant_parameters() if random.randint(0, 4) == 0 else generate_parameters()

def full_tests():
 global GLOBAL_PARAMETERS 
 GLOBAL_PARAMETERS = generate_global_parameters()
 print('GLOBAL_PARAMETERS={}'.format(GLOBAL_PARAMETERS))
...
def main():
 ...
 while True:
  test_number += 1
  random_seed = time.time(); random.seed(random_seed)
  print('\n\ntest_number={}\nrandom_seed={}'.format(test_number, random_seed))  
  overall_score = full_tests()
  #if random.random() < 0.4: overall_score = 99
  if overall_score < 100.: break
